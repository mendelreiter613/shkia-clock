        ...
        <div className={`flex flex-col h-screen w-full relative overflow-hidden transition-colors duration-1000 ${time.isCritical ? 'animate-alarm' : ''}`}>
            ...
            <div className="absolute inset-0 z-10 flex flex-col">
                ...
                <motion.div
                    ...
                    className="w-full flex justify-between items-start p-4 md:p-6"
                >
                ...
                <div className="flex-1 flex flex-col items-center justify-center px-4 w-full">
                    ...
                    {/* Label */}
                    <motion.div className="flex items-center gap-2 mb-4 opacity-80">
                    ...
                    {/* Clock Numbers - FIXED: Reasonable sizing */}
                    <div className="font-clock text-white leading-none relative flex justify-center w-full">
                        {time.isCritical ? (
                            <motion.span
                                ...
                                className="text-[20vw] md:text-[15vw] font-bold text-red-500 drop-shadow-[0_0_50px_rgba(220,38,38,0.8)] tabular-nums"
                            >
                                {time.seconds}
                            </motion.span>
                        ) : (
                            <div className="flex items-center gap-1 md:gap-3">
                                <div className="flex flex-col items-center">
                                    <span className="text-[12vw] md:text-[8rem] font-bold drop-shadow-2xl tabular-nums">{time.hours}</span>
                                    <span className="text-[8px] md:text-sm uppercase tracking-widest opacity-50 font-sans">Hours</span>
                                </div>
                                <span className="text-[8vw] md:text-[6rem] opacity-30 -mt-4">:</span>
                                <div className="flex flex-col items-center">
                                    <span className="text-[12vw] md:text-[8rem] font-bold drop-shadow-2xl tabular-nums">{time.minutes}</span>
                                    <span className="text-[8px] md:text-sm uppercase tracking-widest opacity-50 font-sans">Minutes</span>
                                </div>
                                <span className="text-[8vw] md:text-[6rem] opacity-30 -mt-4">:</span>
                                <div className="flex flex-col items-center">
                                    <span className="text-[12vw] md:text-[8rem] font-medium text-white/80 drop-shadow-xl tabular-nums">{time.seconds}</span>
                                    <span className="text-[8px] md:text-sm uppercase tracking-widest opacity-50 font-sans">Seconds</span>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {!time.isCritical && (
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            className="mt-6 md:mt-10 px-5 py-2.5 rounded-full bg-slate-900/60 backdrop-blur-md border border-white/10 flex items-center gap-2 shadow-2xl max-w-[90vw]"
                        >
                ...
                {/* Footer */}
                <div className="w-full flex justify-center gap-8 md:gap-16 items-end pb-6 pt-2 text-white/40 text-[9px] md:text-xs font-mono uppercase tracking-widest">
